...
function EntryCard({ entry }: { entry: ReasoningEntry }) {
    return (
        <div className={`p-3 rounded-lg glass border-l-4 ${typeColors[entry.type] || 'border-l-gray-500'} transition-all hover:bg-white/5`}>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
                {entries.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-gray-500">
                        ...
                        <p className="text-xs mt-1 opacity-60">The agent's thought process will appear here</p>
                    </div>
                ) : (
                    entries.map((entry) => (
